...
const SearchManager = {
    ...
    toggleSelectAll() {
        ...
        checkboxes.forEach((checkbox, index) => {
            ...
            const buildingData = results[globalIndex];
            ...
            if (buildingData) {
                ...
                if (checkbox.checked) {
                    ...
                    const buildingKey = `${buildingData.빌딩명}_${buildingData.주소}`;
                    if (DataManager.selectedBuildings.has(buildingKey)) {
                        DataManager.toggleBuildingSelection(buildingData);
                    }
                }
        ...
        console.log(`${allChecked ? '해제' : '선택'}: ${checkboxes.length}개 항목`);
    },
    ...
    exportResults() {
        ...
        // CSV 헤더
        const headers = [
            '빌딩명', '주소', '인근역', '공실층', '공실전용면적(평)', 
            '기준층전용면적(평)', '출처회사', 'PDF상태'
        ];
        ...
        const csvData = results.map(item => {
            ...
            return [
                ...
                item.기준층전용면적 || '',
                item.출처회사 || '',
                pdfStatus.available ? `PDF있음(${pdfStatus.confidence}%)` : 'PDF없음'
            ];
        });
        ...
        link.setAttribute('download', `임대정보_검색결과_${new Date().toISOString().slice(0, 10)}.csv`);
        link.style.visibility = 'hidden';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        
        console.log(`📊 CSV 내보내기 완료: ${results.length}개 항목`);
    },
    ...
    saveSearchCriteria() {
        const criteria = {
            ...
            vacancyAreaFrom: document.getElementById('vacancyAreaFrom').value,
            vacancyAreaTo: document.getElementById('vacancyAreaTo').value,
            buildingAreaFrom: document.getElementById('buildingAreaFrom').value,
            buildingAreaTo: document.getElementById('buildingAreaTo').value,
            savedAt: new Date().toISOString()
        };
        
        localStorage.setItem('savedSearchCriteria', JSON.stringify(criteria));
        
        // 성공 메시지
        const toast = this.showToast('검색 조건이 저장되었습니다.', 'success');
        console.log('💾 검색 조건 저장 완료');
    },
    ...
    loadSearchCriteria() {
        try {
            ...
            if (!saved) {
                this.showToast('저장된 검색 조건이 없습니다.', 'info');
                return;
            }
            ...
            // 검색 조건 복원
            document.getElementById('searchType').value = criteria.searchType || 'building';
            this.switchSearchType(criteria.searchType || 'building');
            
            document.getElementById('buildingName').value = criteria.buildingName || '';
            document.getElementById('districtName').value = criteria.districtName || '';
            document.getElementById('stationName').value = criteria.stationName || '';
            document.getElementById('walkingTime').value = criteria.walkingTime || '';
            document.getElementById('vacancyAreaFrom').value = criteria.vacancyAreaFrom || '';
            document.getElementById('vacancyAreaTo').value = criteria.vacancyAreaTo || '';
            document.getElementById('buildingAreaFrom').value = criteria.buildingAreaFrom || '';
            document.getElementById('buildingAreaTo').value = criteria.buildingAreaTo || '';
            
            this.showToast(`검색 조건을 불러왔습니다. (저장일: ${new Date(criteria.savedAt).toLocaleDateString()})`, 'success');
            console.log('📂 검색 조건 불러오기 완료');
            
        } catch (error) {
            console.error('검색 조건 불러오기 실패:', error);
            this.showToast('검색 조건 불러오기에 실패했습니다.', 'error');
        }
    },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        window.devTools = {
            exportResults: () => SearchManager.exportResults(),
            saveCriteria: () => SearchManager.saveSearchCriteria(),
            loadCriteria: () => SearchManager.loadSearchCriteria(),
            showStats: () => PDFManager.showMappingStats()
        };
        console.log('🔧 개발 모드: devTools 객체 사용 가능');
    }
});표시
        this.currentPage = 1;
        this.displayResults();
