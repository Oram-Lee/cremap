...
const DataManager = {
    ...
    search(criteria) {
        ...
        try {
            // vacancies와 buildings 모두 검색하도록 수정
            let searchArray = [];
            
            // vacancies와 buildings 합치기
            if (this.excelData.vacancies && this.excelData.vacancies.length > 0) {
                searchArray = [...this.excelData.vacancies];
            }
            if (this.excelData.buildings && this.excelData.buildings.length > 0) {
                searchArray = [...searchArray, ...this.excelData.buildings];
            }
            
            console.log(`🔍 통합 검색: vacancies(${this.excelData.vacancies?.length || 0}개) + buildings(${this.excelData.buildings?.length || 0}개) = 총 ${searchArray.length}개`);
