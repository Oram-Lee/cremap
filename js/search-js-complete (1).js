...
const SearchManager = {
    ...
    fillEmptyFields(results) {
        ...
        results.forEach(result => {
            ...
            if (result.주소 && result.주소.trim() && !buildingInfo[key].주소) {
                buildingInfo[key].주소 = result.주소;
            }
            if (result.인근역 && result.인근역.trim() && !buildingInfo[key].인근역) {
        ...
        const filledResults = results.map((result, index) => {
            const info = buildingInfo[result.빌딩명] || {};
            ...
            return {
                ...result,
                빌딩명: result.빌딩명 || '-',
                주소: result.주소 || info.주소 || '-',
                인근역: result.인근역 || info.인근역 || '-',
                공실층: result.공실층 || '-',
    ...
    displayResults() {
        ...
        // 빈 필드 채우기
        const filledResults = this.fillEmptyFields(results);
