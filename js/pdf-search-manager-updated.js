...
class PDFSearchManager {
    ...
    // 검색어 추출 (개선된 로직) - 최소한의 영문+숫자 처리만 추가
    getSearchTerms(buildingData) {
        ...
        if (buildingData.빌딩명) {
            // 보이지 않는 문자 제거
            const cleanedName = buildingData.빌딩명.trim().replace(/[\u200B-\u200D\uFEFF]/g, '');
            terms.push(cleanedName);
            
            // 영문+숫자 조합 간단히 처리 (L7 → L 7) - 첫 단어만 확인
            const firstPart = cleanedName.split(/\s/)[0];
            if (/^[A-Za-z]+\d+$/.test(firstPart)) {
                // 첫 단어가 영문+숫자 조합인 경우만 공백 추가 버전 생성
                const spaced = firstPart.replace(/([A-Za-z]+)(\d+)/, '$1 $2');
                const spacedFullName = cleanedName.replace(firstPart, spaced);
                terms.push(spacedFullName);
            }
